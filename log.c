...
void log_command(const char *format, ...) {
	...
	fprintf(MI_DATA->logfile, "%lu ", (unsigned long) 1000000 * timestamp.tv_sec + timestamp.tv_usec);
	vfprintf(MI_DATA->logfile, format, ap);
	fprintf(MI_DATA->logfile, "\n");
}
...
void log_fuse_context(struct fuse_context *context) {
	...
	log_struct(((struct mi_state *)context->private_data), logfile, %08x, );
	log_struct(((struct mi_state *)context->private_data), root_dir, %s, );
